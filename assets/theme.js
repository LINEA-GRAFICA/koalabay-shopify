  ...
  theme.VideoManager = new function () {
    ...
    _._loadYoutubeVideos = function (container) {
      $('.video-container[data-video-type="youtube"]:not(.video--init)', container).each(function () {
        ...
        var $videoElement = $('<iframe class="video-container__video-element" style="-webkit-overflow-scrolling: touch; overflow-y: scroll;position:absolute; top:0px; left:0px; bottom:0px; right:0px; width:100%; height:100%; border:none; margin:0; padding:0; overflow:hidden; z-index:999999;" scrolling="yes" frameborder="0" allowfullscreen="1" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture">').attr({
          id: containerId,
          width: 640,
          height: 360,
  ...
  */
  ...
  var TermsAgreement = class extends HTMLElement {
    connectedCallback() {
      ...
      let termCheckbox = document.querySelector("#term_and_conditions");
      if(termCheckbox){
        
            termCheckbox.addEventListener('invalid', function (event) {
              let errorMessage = window.theme.custom.errorMessage;
              console.log(errorMessage)
              if (event.target.validity.valueMissing) {
                event.target.setCustomValidity(errorMessage);
              }
            });
            termCheckbox.addEventListener('change', function (event) {
    ...
    handleFormSubmittingEvent(evt) {
      ...
      let termCheckbox = document.querySelector("#term_and_conditions_of_product");
          if(termCheckbox){
